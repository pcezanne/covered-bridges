...
all_waypoints = [
	...
	["42.871556, -72.327500","Denman Thompson Bridge, Swanzey"],
...
for num, waypointPair1 in enumerate(all_waypoints, start=0):
    for num2, waypointPair2 in enumerate(all_waypoints, start=0):
        ...
        try:
            route = gmaps.distance_matrix(origins=[waypoint1],
                                          ...
                                          language="English",
                                        	#1754031600000 - August 1,2025 3AM east coast USA, traffic should be very light. Google only takes future times or now

                                          departure_time="1754031600000",
                                          units="metric")
            ...
            waypoint_distances[num, num2] = distance
            print("%s to %s = %s" % (waypoint1, waypoint2, str(distance)))

        except Exception as e:
            print("Error with finding the route between %s and %s. %s" % (waypoint1, waypoint2, e))

# write out the bi-directional matrix
with open("my-bidirectional-dist-dur.py", "w") as out_file:
    out_file.write("p = [\n")

    for num3, waypointPair1 in enumerate(all_waypoints, start=0):
        out_file.write("\t[")

        for num4, waypointPair2 in enumerate(all_waypoints, start=0):

            waypoint1 = waypointPair1[0]
            waypoint2 = waypointPair2[0]
            
            # now lets make sure that A->B is the same as B->A
            # yes, this isn't real world but google maps api can give wildly
            # different durations for the same path, I just want the shortest.
            minDist = min(waypoint_distances[num3, num4], waypoint_distances[num4, num3])
    ...
    #write out the coords array
    out_file.write("\n\ncoords = [\n")
    for num5, waypointPair in enumerate(all_waypoints, start=1):
        out_file.write("\"" + waypointPair[0] + "\", ", )
    out_file.write("\n]\n")
